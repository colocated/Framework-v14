    ...
    */
    async execute(interaction, client, extraArgs) {
        ...
        try {
            ...
            client.logger.error(`Failed to start job ${jobId}: ${error.message}`);
        }

        const { jobEmbed, row } = generateComponents(job);
        await interaction.update({ embeds: [jobEmbed], components: [row] });
        return interaction.followUp({ content: `Job \`${jobId}\` has been started successfully.`, flags: [MessageFlags.Ephemeral] });
    }
};
